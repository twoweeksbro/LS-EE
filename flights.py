...
# 좌석 수와 지연시간 산점도
plt.scatter(merged_df["seats"], merged_df["dep_delay"], alpha=0.3)
plt.show()

plt.scatter(merged_df["seats"], merged_df["arr_delay"], alpha=0.3)
plt.show()
...
# 비행 시간과 지연시간 산점도
plt.scatter(merged_df["air_time"], merged_df["arr_delay"], alpha=0.3, s=3)
plt.show()

plt.scatter(merged_df["air_time"], merged_df["dep_delay"])
plt.show()


plt.figure(figsize=(6, 4))
plt.hist(
    merged_df["dep_delay"], bins=50, edgecolor="black", alpha=0.7
)  # bin: 상자, bins: 상자 몇개?
plt.xlabel("Age")
...
plt.figure(figsize=(10, 10))
sns.heatmap(palne_mat, annot=True, cmap="coolwarm", fmt=".2f", linewidths=0.5)
plt.title("Heatmap of Feature Correlations")
plt.show()
...
plt.figure(figsize=(10, 10))
sns.heatmap(merged_mat, annot=True, cmap="coolwarm", fmt=".2f", linewidths=0.5)
plt.title("Heatmap of Feature Correlations")
plt.show()
...
mosaic(filtered_data, ["model", "origin"], title="mosaic", ax=ax)
plt.show()
...
planes.columns
f_df = flights[
    (lower_boundary <= flights["arr_delay"]) & (flights["arr_delay"] <= upper_boundary)
]


plt.figure(figsize=(6, 4))
plt.hist(
    f_df["arr_delay"], bins=20, edgecolor="black", alpha=0.7
)  # bin: 상자, bins: 상자 몇개?
...
def PreProcessing(df, column):
    ...
    planes.columns
    f_df = flights[
        (lower_boundary <= flights["arr_delay"])
        & (flights["arr_delay"] <= upper_boundary)
    ]
