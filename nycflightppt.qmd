...
2. 결론 : 
좌석 수가 많을수록 비행 시간이 길어지는 경향이 있음
:::

::::

## 데이터를 그룹화 하여 특징 알아내기 {.scrollable}
...
1. 좌석 수(개)별 범주 나눔
- `0~150` (소형 항공기) : 비행시간이 짧은 구간(0~150)에 주로 분포
- `150~250` (중형 항공기) : 비행시간이 100-150인 구간에 주로 분포
- `250~350` (대형 항공기) : 비행시간이 짧은 구간에서는 작은 비율을 차지하다가 긴 구간에서 차지하는 비율이 서서히 증가하는 경향
- `350~450+` (초대형 항공기) : 시각화 자료로 확인 불가할 정도로 적은 수 운행

2. 결론
소형 항공기일 수록 비행 시간에 영향을 많이 받음
- 소형 항공기의 경우 운행 시 비행 시간에 많은 영향을 받음
- 하지만 크기가 큰 항공기일 수록 오히려 비행 시간에 영향을 받지 않고 균등하게 분포하는 경향이 있음
...
1. 좌석 수(개)별 범주 나눔
- `0~150` (소형 항공기) : 비행거리가 짧은 구간(0~1000)에 주로 분포
- `150~250` (중형 항공기) : 비행거리가 1000인 시점부터 전체 빈도 중 차지하는 비율 높아지는 것 관찰 가능
- `250~350` (대형 항공기) : 비행거리 2500부터 운행 빈도가 증가
- `350~450+` (초대형 항공기) : 시각화 자료로 확인 불가할 정도로 적은 수 운행

2. 결론
소형 항공기일 수록 비행 거리에 영향을 많이 받음 (주로 단거리 운행)
- 소형 항공기의 경우 운행 시 비행거리에 많은 영향을 받음
- 하지만 크기가 큰 항공기일 수록 오히려 비행 거리에 영향을 받지 않고 균등하게 분포하는 경향이 있음
...
# Q&A {background-image=./static/images/airport.gif background-opacity=0.3}